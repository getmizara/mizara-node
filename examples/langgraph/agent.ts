 ...
 *
 * This demo simulates the LLM decision based on the environment named
 * in the initial HumanMessage so the example runs without any API key.
 */
function agentNode(state: State): Partial<State> {
  ...
  if (lastMessage instanceof ToolMessage) {
    return {
      messages: [
        new AIMessage({
          content: `Resource deleted successfully. ${lastMessage.content}`,
        }),
      ],
    };
  }
  ...
  // Simulate the LLM deciding to call delete_cloud_resource.
  // The environment comes from the initial human message for the demo.
  const humanMsg = state.messages.find((m) => m instanceof HumanMessage);
  const match = humanMsg?.content.toString().match(/\b(production|staging|development)\b/i);
  const environment = match ? match[1].toLowerCase() : 'staging';
        ...
        tool_calls: [
          {
            ...
            args: { environment, resource_id: 'res_9c21' },
            type: 'tool_call',
          },
        ],
      }),
...
async function authorizationNode(state: State): Promise<Partial<State>> {
  ...
  const result = await mizara.authorize({
    ...
    resource: {
      type: 'cloud_resource',
      id: toolCall.args.resource_id as string,
 ...
 * Replace the simulation here with your real tool call (AWS/GCP/Terraform, etc.)
 */
async function toolsNode(state: State): Promise<Partial<State>> {
  ...
  const simulatedResult = {
    ...
    resource_id: toolCall.args.resource_id,
